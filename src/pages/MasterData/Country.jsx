...
const Country = () => {
  ...
  const handleCreateOrUpdate = async () => {
    try {
      if (editingId !== null) {
        ...
        await axios.put(`${process.env.REACT_APP_API_BASE_URL}/countries/${editingId}`, {
          countryName: form.countryName,
          status: form.status
        });
        const updated = countries.map(c =>
          c.id === editingId ? { ...c, countryName: form.countryName, status: form.status } : c
        );
        setCountries(updated);
      } else {
        // Create new country
        const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/countries`, {
          countryName: form.countryName,
          status: form.status
        });
  ...
  const toggleStatus = async (id) => {
    try {
      ...
      await axios.patch(`${process.env.REACT_APP_API_BASE_URL}/countries/${id}/status`, {
    ...
    <div className="country-container">
      {showModal && (
        <div className="modal-overlay">
          <div className="modal">
            <h3>{editingId !== null ? 'Edit Country' : 'Add New Country'}</h3>
            <input
              type="text"
